...
returns int
begin
declare cod_trabajador int;
select id_t into cod_trabajador from trabajador where fecha_alta > Year(now()-3);
...
begin
declare v_nombre varchar(8);
declare v_tarifa float;
declare v_oficio varchar(30);
declare micursor cursor for select nombre, tarifa, oficio from trabajador where id_t in (select num_annos());
open micursor;
fetch micursor into v_nombre, v_tarifa, v_oficio;
select concat(v_nombre, ' ', v_tarifa, ' ',v_oficio);
close micursor;
End $$
call Antiguedad()$$
drop procedure Antiguedad$$


delimiter $$
create procedure trabajadores_oficio(v_oficio varchar(30))
begin
declare num_emple int;
declare fin boolean default 0;
declare micursor cursor for select count(*) from trabajador where oficio=v_oficio;
declare continue handler for not found set fin =1;
open micursor;
      fetch micursor into num_emple;
      if num_emple > 0 then
      select concat(num_emple, ' ', v_oficio) as 'Nª Trabajador  Oficio';
     
      else 
      select concat('No hay trabajadores de oficio ', oficio);
      end if;
      close micursor;
end $$
#drop procedure trabajadores_oficio;$$
call trabajadores_oficio('Carpintero');$$
